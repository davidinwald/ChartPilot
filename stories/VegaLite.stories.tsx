...
export const Docs: Story = {
  name: "Docs",
  render: () => <div>Vega-Lite documentation and usage tips go here.</div>,
};
  ...
  render: (args) => {
    ...
    try {
      ...
      config.config = {
        ...config.config,
        view: { stroke: null },
        axis: { grid: args.showGrid },
        legend: { orient: "top" },
      };

      if (args.theme === "dark") {
        config.config.background = "#333";
        config.config.axis = {
          ...config.config.axis,
          domainColor: "#fff",
          gridColor: "#444",
      ...
      return (
        <VegaLiteChart spec={config} width={args.width} height={args.height} />
      );
    } catch (error) {
  ...
  render: (args) => {
    ...
    try {
      ...
      if (args.innerRadius > 0) {
        config.mark = { ...config.mark, innerRadius: args.innerRadius };
      }
...
export const ScatterPlot: StoryObj<BaseChartArgs & { pointSize: number }> = {
  ...
  render: (args) => {
    ...
    try {
      ...
      config.mark = { ...config.mark, size: args.pointSize };
